...
export default function FundingProcess() {
  return (
    <section className="max-w-7xl mx-auto px-6 py-20">
      <h2 className="text-3xl font-bold text-center mb-3" style={{ color: "#2D2754" }}>Funding Process</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {steps.map((s) => (
          <div key={s.num} className="flex gap-4 p-6 rounded-2xl hover:shadow-lg transition" style={{ border: "1px solid rgba(50,225,252,0.2)" }}>
            <span className="text-3xl font-extrabold" style={{ color: "rgba(46,44,119,0.2)" }}>{s.num}</span>
            <div>
              <h3 className="font-bold mb-1" style={{ color: "#2D2754" }}>{s.title}</h3>
